...
import { getMountRecord } from "../API";
...
import MountNode from "./Node/components/MountNode";
      ...
      <div className="record_content">
        <div>
          <div className="title items">
            ...
            <div className="item">{t("Mount Node")}</div>
            <div className="item">{t("End Time")}</div>
            ...
            {RecordList3?.records?.length > 0 ? (
              RecordList3?.records?.map((item: any, index: any) => (
                <div key={index} className="content items">
                  <div className="item">
                    {dateFormat("mm/dd", new Date(item?.startTime))}
                  </div>
                  <div
                    className="item"
                  ...
                  >
                    {item?.nodeAddress?.length > 9
                      ? AddrHandle(item?.nodeAddress, 5, 4)
                      : item?.nodeAddress}
                  </div>
                  <div className="item">
                    {!!item?.endTime
                      ? dateFormat("mm/dd", new Date(item?.endTime))
                      : "-"}
                  </div>
        ...
        <MountNode
          ...
          fun={getInitData}
          ShowTipModal={UnMountNodeState}
          type={2}
          close={() => {
            setUnMountNodeState(false);
          }}
        />
      </div>
    </div>
