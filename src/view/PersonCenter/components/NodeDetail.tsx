...
import { AddrHandle, NumSplic1, addMessage } from "../../../utils/tool";
import { getNodeData, getUserNodeData } from "../../../API";
...
export default function ModalContent(props: any) {
  ...
  const getRecordFun = () => {
    getUserNodeData().then((res: any) => {
      setNodeData(res?.data || {});
    });
  };

  useEffect(() => {
    if (!!token && props?.ShowTipModal) {
      getRecordFun();
    } else {
      // setRecordList3({});
    }
      ...
      <ModalContainer>
        ...
        <ModalContainer_Content>
          <div className="items">
            <div className="item">
              {t("挂载地址")}：{" "}
              <span>{AddrHandle(NodeData?.address, 6, 6)}</span>
            </div>
            <div className="item">
              {t("挂载人数")}： <span>{NodeData?.mountUserSize}</span>
            </div>
            <div className="item">
              {t("质押总量")}：{" "}
              <span>
                <img src={coin_logo} alt="" /> {NodeData?.pledgeNum}
              </span>
            </div>
          </div>
        </ModalContainer_Content>
