...
const rewardType = { 1: "收益发放", 2: "质押退回", 3: "领取收益" };
export default function Rank() {
  ...
  useEffect(() => {
    if (!!token && !!stateObj?.type) {
      getInitData();
    } else {
    }
  }, [web3ModalAccount, token, PageNum, stateObj?.type, ActiveTab]);

  return (
    <div className="userlist_record">
      ...
      <div className="record_content">
        <div>
          <div className="title items">
            <div className="item">{t("节点地址")}</div>
            <div className="item">{t("我的质押数(Token)")}</div>
            <div className="item">{t("节点总数(Token)")}</div>
          </div>
            ...
            {[{}, {}]?.length > 0 ? (
              [{}, {}]?.map((item: any, index: any) => (
                <div key={index} className="content items">
                  <div className="item">0xhj...3048</div>
                  <div className="item">1000000</div>
