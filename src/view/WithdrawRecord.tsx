...
import { ContractUrl, contractAddress } from "../config";
...
import { aepWithdrawRecord } from "../API";
...
export default function Rank() {
  ...
  const stateObj: any = {
    "-1": { title: "143", color: "#E34956" },
    0: { title: "140", color: "#000" },
    1: { title: "141", color: "#000" },
    2: { title: "142", color: "#00D558" },
    3: { title: "143", color: "#E34956" },
  };
  const getInitData = () => {
    aepWithdrawRecord({
      orderType: OrderType,
  ...
  }, [web3ModalAccount, token, PageNum, OrderType, InputAddress]);
          ...
          <div className="search_box">
            <div>
              ...
              <input
                type="text"
                placeholder={t("39")}
                value={!!InputAddress ? InputAddress : ""}
                onChange={(e: any) => {
                ...
                <div key={index} className="item">
                  <div className="item_top">
                    <div className="amount">
                      {t("42")}：{item?.fromNum} {item?.coinName}
                    </div>

                    <div
                      className="status"
                      style={{
                        color: stateObj[item?.status]?.color,
                      }}
                    >
                      {t(stateObj[item?.status]?.title)}
                    </div>
                  </div>
                  <div className="time">
                    {t("44")}：
                  ...
                  <div className="address">
                    {t("45")}：
                    <span
                      style={{ color: item?.status === 2 ? "#0140E7" : "#000" }}
                      onClick={() => {
                        window.open(ContractUrl + item?.toAddress);
                      }}
                    >
                      {AddrHandle(item?.toAddress, 6, 10)}
                    </span>
                  </div>
                </div>
              ))}
