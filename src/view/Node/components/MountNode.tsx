...
import { cancelMountNode, mountNode } from "../../../API";
...
export default function ModalContent({
  ...
  const ConfirmFun = async (address: any, fun1: any) => {
    if (web3ModalAccount) {
      await signFun(
        (res: any) => {
          fun1({
            ...
            if (res.code === 200) {
              ...
              fun();
              close();
              return addMessage(type === 1 ? t("挂载成功") : t("取消挂载成功"));
            } else {
              showLoding(false);
              addMessage(res.msg);
            }
      ...
      <ModalContainer>
        ...
        <ModalContainer_Content>
          <div className="tip_box">
            {type === 1
              ? t("Confirm Mounting This Node")
              : t("Unmount this node")}
            {type === 1 ? (
              <div className="address">
                {CurrentItem?.userAddress?.length > 11
                  ? AddrHandle(CurrentItem?.userAddress, 6, 5)
                  : CurrentItem?.userAddress}
              </div>
            ) : (
              <div className="address">
                {CurrentItem?.nodeAddress?.length > 11
                  ? AddrHandle(CurrentItem?.nodeAddress, 6, 5)
                  : CurrentItem?.nodeAddress}
              </div>
            )}
          </div>
          <div
            ...
            onClick={() => {
              ConfirmFun(
                type === 1
                  ? CurrentItem?.userAddress
                  : CurrentItem?.nodeAddress,
                type === 1 ? mountNode : cancelMountNode
              );
            }}
          >
            {t("Confirm")}
